...
def evaluate_isophonics_wav(csvF, wfilename):
    ...
    theirData = open("AnnotatedData.txt","w")
    theirData.write('annotated data: ')
    theirData.write(str(chords))
    ourData = open("ProjectedData.txt","w")
    ourData.write('projected data: ')
    ourData.write(str(data))
    theirData.close()
    ourData.close()
txt = './chords/stl.txt'
wav = './chords/stl.wav'
